...
def obtener_socio(dni):
    ...
    if socio:
        return jsonify({
            'dni': socio.dni,
            'nomyape': socio.nomyape,
            'sexo': socio.sexo,
            'categoria': socio.categoria,
            'email': socio.email,
            'telefono': socio.tel,
            'direccion': socio.direccylocalidad
        }), 200
...
def dar_alta_socio():
    ...
    nomyape = request.json.get('nomyape')
    sexo = request.json.get('sexo')
    categoria = request.json.get('categoria')
    email = request.json.get('email')
    tel = request.json.get('tel')
    direccylocalidad = request.json.get('direccylocalidad')
    return administracion.dar_alta_socio(dni, nomyape, sexo, categoria, email, tel, direccylocalidad)
...
def modificar_socio(dni):
    nuevo_nomyape = request.json.get('nomyape')
    nuevo_sexo = request.json.get('sexo')
    nueva_categoria = request.json.get('categoria')
    nuevo_email = request.json.get('email')
    nuevo_tel = request.json.get('tel')
    nuevo_direccylocalidad = request.json.get('direccylocalidad')
    return administracion.actualizar_socio(dni, nuevo_nomyape, nuevo_sexo, nueva_categoria, nuevo_email, nuevo_tel, nuevo_direccylocalidad)
